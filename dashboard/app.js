...
function hideSecondarySections(){['Same-input FD implementation check','Period-specific S3 error','S3 candidate comparison'].forEach(title=>{const h=[...document.querySelectorAll('h2')].find(x=>x.textContent.trim()===title);if(h)h.closest('section').hidden=true;});}
function makeClosureSpeedChart(link){const source=(window.CLOSURE_RECONSTRUCTION_DATA||[]).find(x=>x.key===link.id);if(!source){document.querySelector('#closureSpeedChart').innerHTML='<p class="table-note">Holdout reconstruction is not available for this link.</p>';return;}const labels=source.rows.map(r=>r.time);makeSvg('closureSpeedChart',[{name:'observed speed',values:source.rows.map(r=>r.observedSpeed),color:colors.blue,labels},{name:'reconstructed speed',values:source.rows.map(r=>r.reconstructedSpeed),color:colors.orange,labels}]);}
...
function render(link){
  ...
  const holdout=(window.CLOSURE_HOLDOUT_DATA||[]).filter(r=>r.key===link.id).sort((a,b)=>['NT1','AM','MD','PM','NT2'].indexOf(a.period)-['NT1','AM','MD','PM','NT2'].indexOf(b.period));
  document.querySelector('#closureHoldoutStats').innerHTML=holdout.map(r=>`<tr><th>${r.period}</th><td>${fmt(r.volumeErrorPct,1)}</td><td>${fmt(r.flowMae,0)}</td><td>${fmt(r.flowRmse,0)}</td><td>${fmt(r.speedMae,2)}</td><td>${fmt(r.speedR2,3)}</td><td>${fmt(r.durationH,2)}</td></tr>`).join('');
...
hideSecondarySections(); data.links.forEach(l=>{const o=document.createElement('option');o.value=l.id;o.textContent=`${l.linkId} · TMC ${l.tmcId}`;select.appendChild(o);}); select.addEventListener('change',()=>render(data.links.find(l=>l.id===select.value))); render(data.links[0]);
